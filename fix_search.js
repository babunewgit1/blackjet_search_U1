...
function initialize() {
  ...
  if (way === "one way") {
    ...
    const validTimestamp = ensureValidDate(
      sessionData.timeStamp,
      sessionData.dateAsText
    );

    if (!validTimestamp) {
      console.error("Invalid date data:", sessionData);
      alert("Invalid date data. Please try again.");
      return;
    }
    ...
    data = {
      ...
      pax: sessionData.pax,
      date: sessionData.timeStamp * 1000, // Convert back to milliseconds for API
    };
    apiUrl = apiUrlOneWay;
  } else if (way === "round trip") {
    const sessionData = JSON.parse(storedData);
    const validOutboundTimestamp = ensureValidDate(
      sessionData.timeStamp,
      sessionData.dateAsText
    );
    const validReturnTimestamp = ensureValidDate(
      sessionData.timeStampReturn,
      sessionData.returnDateAsText
    );

    if (!validOutboundTimestamp || !validReturnTimestamp) {
      console.error("Invalid date data:", sessionData);
      alert("Invalid date data. Please try again.");
      return;
    }
    ...
    data = {
      ...
      "out-dep date": sessionData.timeStamp * 1000, // Convert back to milliseconds
      "ret-date": sessionData.timeStampReturn * 1000, // Convert back to milliseconds
    ...
    // Handle array of timestamps for multi-city
    const validTimestamps = sessionData.timeStamp.map((ts, index) =>
      ensureValidDate(ts, sessionData.dateAsText[index])
    );

    if (validTimestamps.some((ts) => !ts)) {
      console.error("Invalid date data:", sessionData);
      alert("Invalid date data. Please try again.");
      return;
    }
    ...
    data = {
      "from airport id": sessionData.fromId,
      "to airport id": sessionData.toId,
      date: validTimestamps.map((ts) => ts * 1000), // Convert each timestamp back to milliseconds
      pax: sessionData.pax,
      date_as_text: sessionData.dateAsText,
      time_as_text: sessionData.timeAsText,
    };
